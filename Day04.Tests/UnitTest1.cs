...
".Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
    ...
    {
        var rawInput = FileHelper.ExtractInputFromFile("../../../../Day04/bin/Debug/net6.0/day4input.txt");
        ...
        [Fact]
    public void RunningGameUntilFinalWinProducesExpectedLastNumber()
    {
                var positionListOne = new List<Position>();
        positionListOne.AddRange(Position
            .GeneratePositionRow(new List<int>() { 22, 13, 17, 11, 0 }, 0));
        ...
        var finalState = GameOperations.RunGameUntilFinalWin(initialStep);
        var finalCalledNumber = finalState.LastCalledNumber;
        
        Assert.Equal(13, finalCalledNumber);

        var winningBoard = GameOperations.GetWinningBoards(finalState).Single();

        var totalUncalledValuesFromWinningBoard = winningBoard.TotalUncalledValues;
        
        Assert.Equal(148, totalUncalledValuesFromWinningBoard);
        
        Assert.Equal(1924, totalUncalledValuesFromWinningBoard * finalCalledNumber);
    }
}